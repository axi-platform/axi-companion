...
import {humanFileSize, getFileInfo} from './util'
    ...
    <Button className={s.ripple}>
      <Paper title="No Files Yet -- Upload Something.">
        <div className={s.noFiles}>
          ...
          <h2 className={s.h}>Let&apos;s Upload some Files.</h2>
          <h3 className={s.h}>Use the Upload Menu to upload your files.</h3>
        </div>
      </Paper>
    </Button>
  </Upload>
...
const Preview = ({name, type, path, preview}) => {
    ...
    return (
      <div>
        ...
        <Paper title={`File Information`}>
          ...
          <div>
            <small className={s.small}>ID: {file.id}</small>
          </div>
        </Paper>
      </div>
    )
